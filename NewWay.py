...
class ErrorMessage(tk.Toplevel):
    def __init__(self, signal, mode):
        ...
        if self.mode == 0:
            ...
            pad_text = 'Warning '
        self.error = tk.Message(self, text=pad_text + self.signal,
...
class Affine(tk.Frame):
    def __init__(self, parent):
        ...
        self.plain_label = tk.Label(self.inputs, text='Nhập một đoạn văn bản (IN HOA): ', font=('Times New Roman', 13))
        ...
        self.keys.pack()
        self.group.pack()
        self.group['background'] = background_color
        self.inputs['background'] = background_color
        self.keys['background'] = background_color
        self.intro['background'] = background_color
        self.plain_label['background'] = background_color
        self.cipher_label['background'] = background_color
        self.key_label_a['background'] = background_color
        self.key_label_b['background'] = background_color
...
class EncryptSYMWindow(tk.Toplevel):
    ...
    def encrypt_file_ECB(self, file_name, key):
        ...
        _bool = save_file(enc, 'wb', 'Save as',
                    (("All files", "*.*"),
    ...
    def copy(self):
        if self.parent.get_func() == 1:
            if self.file_val.get() == 0:
                ...
                self.parent.clipboard.set_link(self.get_link())
        else:
            self.parent.clipboard.set_plain(self.get_plain())
            self.parent.clipboard.set_cipher(self.get_cipher())
    ...
    def paste(self):
        if self.parent.get_func() == 1:
            if self.file_val.get() == 0:
                ...
                self.set_link(self.parent.clipboard.get_link())
        else:
            self.set_plain(self.parent.clipboard.get_plain())
...
class DecryptSYMWindow(tk.Toplevel):
    def __init__(self, parent):
        self.parent = parent
        tk.Toplevel.__init__(self)
        self.title("Chương trình giải mã đối xứng")
        self.geometry('600x360')
        self.frame = tk.Frame(self)
        self.frame.pack()
        self.group = tk.LabelFrame(self)
    ...
    def decrypt_file_ECB(self, file_name, key):
        ...
        _bool = save_file(dec, 'wb', 'Save as',
                            (("All files", "*.*"),
                            ("Text files", "*.txt"),
                            ("Png files", "*.png"),
                            ("Image files", "*.jpg")), ".txt")
...
class Symmetric(tk.Frame):
    def __init__(self, parent):
        ...
        self.explain = tk.Label(self.group, text="Ta dùng đồng thời cùng một key để mã hóa và giải mã"
                                , font=("Times New Roman", 13))
        self.explain.pack()
        self.func_group = tk.LabelFrame(self.group, borderwidth=0, highlightthickness=0
        ...
        self.mode_group = tk.LabelFrame(self.group, text="Chọn chế độ mã hóa", borderwidth=0, highlightthickness=0
                                        , font=("Times New Roman", 13))
        ...
        self.group['background'] = background_color
        self.explain['background'] = background_color
        self.des_func['background'] = background_color
        self.aes_func['background'] = background_color
        self.cbc['background'] = background_color
        self.ecb['background'] = background_color
        self.mode_group['background'] = background_color
        self.func_group['background'] = background_color
...
class EncryptASYMWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.cipher_text = tk.Entry(self.frame, width=50)
        self.cipher_text.grid(column=1, row=1)
        self.link_label = tk.Label(self.frame, text="Tệp tin chứa khóa công khai : "
                                   , font=("Times New Roman", 13))
    ...
    def generate_key(self):
        ...
        _bool_pri = save_file(key.exportKey('PEM'), 'wb', 'Lưu khóa cá nhân',
                     (("All files", "*.*"),
                      ("PEM files", "*.pem")), ".pem")
        while True:
            try:
                ...
                ErrorMessage('04', 1)
                raise
        _bool_pub = save_file(key.publickey().exportKey('PEM'), 'wb'
...
class DecryptASYMWindow(tk.Toplevel):
    def __init__(self, parent):
        self.parent = parent
        tk.Toplevel.__init__(self)
        self.title("Chương trình giải mã bất đối xứng")
        self.geometry('600x360')
        self.frame = tk.Frame(self)
        self.frame.pack()
        self.cipher_label = tk.Label(self.frame, text="Bản mã: "
        ...
        self.plain_text = tk.Entry(self.frame, width=50)
        self.plain_text.grid(column=1, row=1)
        self.link_label = tk.Label(self.frame, text="Tệp tin chứa khóa cá nhân: "
                                   , font=("Times New Roman", 13))
        self.link_label.grid(column=0, row=2, sticky="W")
        self.link_text = tk.Entry(self.frame, width=50)
        self.link_text.grid(column=1, row=2)
    ...
    def decrypt(self):
        while True:
            try:
                ...
                while True:
                    try:
                        cipher = bytearray.fromhex(self.get_cipher())
                        break
                    except ValueError:
                        ErrorMessage('08', 0)
                        raise
                ciphertext = plaintext.decrypt(cipher, 'sentinel')
                while True:
                    try:
...
class Asymmetric(tk.Frame):
    def __init__(self, parent):
        ...
        self.group['background'] = background_color
        self.explain['background'] = background_color
        self.diffie_hellman_func['background'] = background_color
        self.rsa_func['background'] = background_color
        self.func_group['background'] = background_color
...
class Hashing(tk.Frame):
    def __init__(self, parent):
        ...
        self.explain = tk.Label(self.group,
                                text="Ta băm một đoạn kí tự hoặc file ra một đoạn kí tự, không thể giải mã (ngoài MD5)"
                                , font=("Times New Roman", 13))
        self.explain.pack()
        self.func_group = tk.LabelFrame(self.group, borderwidth=0, highlightthickness=0, text="Chọn thuật toán"
                                        , font=("Times New Roman", 13))
        ...
        self.group['background'] = background_color
        self.explain['background'] = background_color
        self.md5_func['background'] = background_color
        self.sha1_func['background'] = background_color
        self.sha256_func['background'] = background_color
        self.sha224_func['background'] = background_color
        self.sha384_func['background'] = background_color
        self.sha512_func['background'] = background_color
        self.func_group['background'] = background_color
...
class SigningWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.frame = tk.Frame(self)
        self.frame.pack()
        self.group = tk.LabelFrame(self.frame, borderwidth=0, highlightthickness=0)
        self.group.pack()
    ...
    def generatesignature(self):
        ...
        if self.func_val.get() == 0:
            hashing_text = SHA1.new(plaintext)
        elif self.func_val.get() == 1:
            hashing_text = MD5.new(plaintext)
        elif self.func_val.get() == 2:
            hashing_text = SHA256.new(plaintext)
        elif self.func_val.get() == 3:
            hashing_text = SHA224.new(plaintext)
        elif self.func_val.get() == 4:
            hashing_text = SHA384.new(plaintext)
        else:
            hashing_text = SHA512.new(plaintext)
        cipher = PKCS1_v1_5_2.new(key)
        while True:
            try:
                signature = cipher.sign(hashing_text)
                break
            except TypeError:
                ErrorMessage('23', 0)
                raise
        _bool = save_file(signature, 'wb', 'Save as',
                  (("All files", "*.*"),
                   ("PEM files", "*.sig")), ".sig")
        while True:
            try:
...
class VerifyingWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.link_button_file = tk.Button(self.group, text="Chọn tệp", command=self.browse_file)
        self.link_button_file.grid(column=2, row=0)
        self.link_label_certification = tk.Label(self.group, text="Đường dẫn chứng thư: "
                                       , font=("Times New Roman", 13))
        self.link_label_certification.grid(column=0, row=1, sticky="W")
        self.link_text_certification = tk.Entry(self.group, width=50)
        self.link_text_certification.grid(column=1, row=1)
        self.link_button_certification = tk.Button(self.group, text="Chọn tệp", command=self.browse_certification)
        self.link_button_certification.grid(column=2, row=1)
    ...
    def get_link_certification(self):
        while True:
            try:
                if self.link_text_certification.get() == '':
                    raise ValueError('Value Error')
                linktext = self.link_text_certification.get()
                break
            except ValueError:
                ErrorMessage('27', 0)
                raise
    ...
    def set_link_certification(self, text):
        self.link_text_certification.delete(0, tk.END)
        self.link_text_certification.insert(tk.INSERT, text)
    ...
    def verify_file(self):
        ...
        if self.func_val.get() == 0:
            ...
            digest = 'sha512'
        try:
            x509 = crypto.load_certificate(crypto.FILETYPE_PEM, open(self.get_link_certification(), 'rb').read())
            key_object = x509.get_pubkey()
            key_bytes = crypto.dump_publickey(crypto.FILETYPE_PEM, key_object)
            key = crypto.load_publickey(crypto.FILETYPE_PEM, key_bytes)
            x509 = crypto.X509()
            x509.set_pubkey(key)
            if crypto.verify(x509, signature, plaintext, digest) is None:
                AcceptMessage()
            else:
                raise Exception('ss')
        except FileNotFoundError:
    ...
    def browse_certification(self):
        self.set_link_certification(browse_file())
...
class DigitalSignature(tk.Frame):
    def __init__(self, parent):
        ...
        self.group.pack(side=tk.TOP, expand=True, fill=tk.BOTH)
        self.explain = tk.Label(self.group,
                                text="Ký chữ ký số cho tệp tin và kiểm tra tính toàn vẹn của một tệp tin có chữ ký"
                                , font=("Times New Roman", 13))
        self.explain.pack()
...
class Menu(tk.Toplevel):
    def __init__(self, parent):
        ...
        "Lỗi 02: Không được bỏ trống ô khóa nào, mỗi ô khóa phải là số nguyên tố\n\n"
        "Lỗi 03: Chưa nhập bản gốc\n\n"
        "Lỗi 04: Bản gốc phải là kí tự ASCII in hoa\n\n"
        "Lỗi 05: Chưa nhập bản mã\n\n"
        "Lỗi 06: Bản mã phải là kí tự ASCII in hoa\n\n"
        "\n***\n"
        "Các lỗi khi dùng mã hóa đối xứng:\n\n"
        "Chú ý: Chương trình không sử dụng kiểu mã hóa DES để mã hóa và giải mã tệp do vấn đề bảo mật\n\n"
        "Cảnh báo 01: Lưu tệp tin mã hóa không thành công\n\n"
        "Cảnh báo 02: Lưu tệp tin giải mã không thành công\n\n"
        "Lỗi 01: Chưa nhập khóa\n\n"
        "Lỗi 03: Chưa nhập bản gốc\n\n"
        "Lỗi 05: Chưa nhập bản mã\n\n"
        "Lỗi 07: Yêu cầu kiểm tra lại độ dài khóa\n\n"
        "Lỗi 08: Yêu cầu kiểm tra lại độ dài bản mã\n\n"
        "Lỗi 09: Bản mã sai\n\n"
        "Lỗi 10: Chưa nhập iv, bạn bắt buộc nhập iv ở chế độ CBC\n\n"
        "Lỗi 11: Yêu cầu kiểm tra lại độ dài iv\n\n"
        "Lỗi 12: Khóa sai\n\n"
        "Lỗi 13: Chưa nhập đường dẫn tệp tin\n\n"
        "Lỗi 14: Không tìm thấy tệp tin\n\n"
        "Lỗi 15: Bản mã không được mã hóa bởi chế độ ECB\n\n"
        "Lỗi 16: Đang sử dụng mã khóa của kiểu mã hóa khác (thường gặp ở chế độ ECB)\n\n"
        "Lỗi 17: Đang sử dụng mã khóa hoặc iv của kiểu mã hóa khác (thường gặp ở chế độ CBC)\n\n"
        "Lỗi 18: Tệp tin quá lớn, thường là lớn hơn 170MB\n\n"
        "Lỗi 19: Bản mã hoặc khóa không đúng\n\n"
        "\n***\n"
        "Các lỗi khi dùng mã hóa bất đối xứng:\n\n"
        "Cảnh báo 03: Phiên bản hiện tại chưa có chế độ mã hóa bằng difie-hellman, dự kiến sẽ được update ở phiên bản sau\n\n"
        "Cảnh báo 04: Lưu khóa cá nhân không thành công\n\n"
        "Cảnh báo 05: Lưu khóa công khai không thành công\n\n"
        "Lỗi 03: Chưa nhập bản gốc\n\n"
        "Lỗi 05: Chưa nhập bản mã\n\n"
        "Lỗi 08: Yêu cầu kiểm tra lại độ dài bản mã\n\n"
        "Lỗi 09: Bản mã sai\n\n"
        "Lỗi 13: Chưa nhập đường dẫn tệp tin\n\n"
        "Lỗi 14: Không tìm thấy tệp tin\n\n"
        "Lỗi 20: Chưa nhập đường dẫn tệp tin chứa khóa\n\n"
        "Lỗi 21: Không tìm thấy tệp tin chứa khóa\n\n"
        "Lỗi 22: Chỉ giải mã bằng khóa cá nhân\n\n"
        "\n***\n"
        "Các lỗi khi băm:\n\n"
        "Lỗi 03: Chưa nhập bản gốc\n\n"
        "Lỗi 13: Chưa nhập đường dẫn tệp tin\n\n"
        "Lỗi 14: Không tìm thấy tệp tin\n\n"
        "\n***\n"
        "Các lỗi khi tạo chữ kí số:\n\n"
        "Cảnh báo 06: Lưu chữ ký số không thành công\n\n"
        "Lỗi 13: Chưa nhập đường dẫn tệp tin\n\n"
        "Lỗi 14: Không tìm thấy tệp tin\n\n"
        "Lỗi 20: Chưa nhập đường dẫn tệp tin chứa khóa\n\n"
        "Lỗi 21: Không tìm thấy tệp tin chứa khóa\n\n"
        "Lỗi 23: Chỉ ký bằng khóa cá nhân\n\n"
        "Lỗi 24: Định dạng khóa sai\n\n"
        "\n***\n"
        "Các lỗi khi tạo chứng thư số:\n\n"
        "Cảnh báo 07: Lưu chứng thư không thành công\n\n"
        "Cảnh báo 08: Lưu khóa cá nhân không thành công\n\n"
        "Lỗi 25: Không được bỏ trống ô thông tin nào\n\n"
        "Lỗi 26: Nhập ô quốc gia chỉ với hai kí tự\n\n"
        "\n***\n"
        "Các lỗi khi xác minh tệp tin:\n\n"
        "Lỗi 13: Chưa nhập đường dẫn tệp tin\n\n"
        "Lỗi 14: Không tìm thấy tệp tin\n\n"
        "Lỗi 27: Chưa nhập đường dẫn tệp tin chứa chứng thư (định dang .crt)\n\n"
        "Lỗi 28: Không tìm thấy tệp tin chứa chứng thư (định dang .crt)\n\n"
        "Lỗi 29: Chưa nhập đường dẫn tệp tin chứa chữ ký số\n\n"
        "Lỗi 30: Không tìm thấy tệp tin chứa chữ ký số\n\n"
        "Lỗi 31: Không đúng thuật toán băm hoặc định dang các tệp tin\n\n"
...
class MainWindow(tk.Frame):
    def __init__(self, parent):
        ...
        self.affine['background'] = background_color
        self.welcome['background'] = background_color
